  ...
  return (
    <div className='bg-gray-50 py-20 p-4 space-y-10'>
        ...
        <div className='flex items-end justify-center gap-7'>
            {data.map((item, index) => (
            <Card key={item.title} className={`${index == 2 && "!bg-blue-500 !text-white h-78"} ${index == 0 && "h-64"} ${index === 1 && "h-72"} group shadow-none border-solid hover:bg-blue-600 transition-all hover:scale-110 duration-700 bg-white hover:!text-white text-black lg:max-w-2xs border`}>
                ...
                <CardFooter className={`group-hover:text-white  text-lg  flex justify-between`}>
                    <span className={`${index != 2 && "text-muted-foreground group-hover:text-white"} text-sm`}>Click on the arrow to know more</span>
                    <Link href={item.url}>
